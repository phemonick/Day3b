...
var tester=require('./prototype.js');
Array.prototype =tester;
...
describe('Binary Search to traverse an ordered list, effectively', function() {
  describe('Populate the arrays with valid content', function() {
    ...
    it('should create an array from 1 to 20, with intervals of 1', function() {
      expect(tester.oneToTwenty[0]).toBe(1);
      expect(tester.oneToTwenty[19]).toBe(20);
      expect(tester.oneToTwenty.length).toBe(20);
      for(var i = 0; i < oneToTwenty.length - 1;i++) {
        expect(tester.oneToTwenty[i + 1] - oneToTwenty[i]).toBe(1);
      }
    });

    it('should create an array from 2 to 40, with intervals of 2', function() {
      expect(tester.twoToForty[0]).toBe(2);
      expect(tester.twoToForty[19]).toBe(40);
      expect(tester.twoToForty.length).toBe(20);
      for(var i = 0; i < twoToForty.length - 1;i++) {
        expect(tester.twoToForty[i + 1] - twoToForty[i]).toBe(2);
      }
    });

    it('should create an array from 10 to 10000, with intervals of 10', function() {
      expect(tester.tenToOneThousand[0]).toBe(10);
      expect(tester.tenToOneThousand[99]).toBe(1000);
      expect(tester.tenToOneThousand.length).toBe(100);
      for(var i = 0; i < tenToOneThousand.length - 1;i++) {
        expect(tester.tenToOneThousand[i + 1] - tenToOneThousand[i]).toBe(10);
      }
    });
  });
